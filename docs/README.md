...
| [`09-m6-implementation-log.md`](09-m6-implementation-log.md) | M6 complete: the learning loop -- `@pros/miner` (deterministic correction mining + session cards + pr-link/plan-artifact-gated intent clustering + loop proposals), `@pros/review`'s "new to you" vocabulary check, and the dashboard's `/loops` proposal page. Verified against the real, extracted `history.jsonl`: 309 corrections found, the mothership triage cluster rediscovered (90 sessions, 54 gated). |
...
| [`11-project-status.md`](11-project-status.md) | **Read this first if you're picking the project back up.** The whole-project status doc: what the system does end to end, how to run every piece, every env var you must configure, every known gap consolidated in one place, and the standing verification checks. |
| [`12-cleanup-log.md`](12-cleanup-log.md) | Post-M7 cleanup pass: the real root cause of the `@pros/barrier` guardian kill-test #2 failure (a genuine implementation bug, not the environment flake earlier milestones assumed), closing the Gate 1 -> Gate 2 continuation gap (`pros implement`, the scheduled continuation job), and fixing `PROS_SKILL_LOCK_FILE`'s wrong default. |
| [`13-zero-token-rework.md`](13-zero-token-rework.md) | Credential-simplification rework: trigger sources go MCP-first (Linear/Slack/Granola, API-key kept as a headless fallback), notifications default to Slack via MCP (ntfy kept as an alternative transport), and draft PRs default to the operator's own ambient `gh auth` session with the model subprocess given no GitHub credential at all (the scoped `PROS_GH_PR_TOKEN` path stays, as strictly stronger opt-in hardening). The resulting must-configure list is credential-free. |
| [`14-design-system.md`](14-design-system.md) | The dashboard's visual overhaul: dark/"paper" design system (Tailwind + shadcn/ui), the Silk WebGL shader background and its accessibility fallbacks, the component inventory, the Linear-style sessions board + `/new` trigger front door, and the full page-by-page information architecture. |
